    ...
    BaseLib::ConfigTree const& config)
{
    ...
    //! \ogs_file_param_special{material__solid__constitutive_relation__Weibull__damage_properties__m_w}
    auto& m_w =
        ProcessLib::findParameter<double>(config, "m_w", parameters, 1);

    DBUG("Use \'%s\' as m_w.", m_w.name.c_str());

    //! \ogs_file_param_special{material__solid__constitutive_relation__Weibull__damage_properties__r_d}
    auto& r_d = ProcessLib::findParameter<double>(config, "r_d", parameters, 1);

    DBUG("Use \'%s\' as r_d.", r_d.name.c_str());

    return std::unique_ptr<WeibullDamageProperties>{
        new WeibullDamageProperties{alpha_d, m_w, r_d}};
}
