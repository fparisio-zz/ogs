...
template <int DisplacementDim>
double yieldFunction(MaterialPropertiesV const& mp,
                     PhysicalStressWithInvariants<DisplacementDim> const& s,
                     double const k)
{
...
std::tuple<typename SolidEhlers<DisplacementDim>::KelvinVector,
           ...
           std::unique_ptr<typename SolidEhlers<DisplacementDim>::KelvinMatrix>>
        ...
        material_state_variables)
{
    ...
    MaterialPropertiesV const mp(t, x, _mp);
    ...
    auto tangentStiffness = std::unique_ptr<KelvinMatrix>{new KelvinMatrix};
    ...
    else
    {
        ...
        if (auto&& solution = newton<JacobianResidualSize>(
            ...
            return std::make_tuple(sigma, nullptr, nullptr);
