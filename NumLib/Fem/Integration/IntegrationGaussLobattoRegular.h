...
namespace NumLib
{
/// Gauss-Lobatto quadrature rule for regular shape elements: line, quad and
/// hex.
///
/// \tparam N_DIM    Spatial dimension
template <unsigned N_DIM>
class IntegrationGaussLobattoRegular
{
    using WeightedPoint =
        typename MathLib::TemplateWeightedPoint<double, double, N_DIM>;

public:
    /// Create IntegrationGaussLobattoRegular of the given Gauss-Lobatto
    /// integration order.
    ///
    /// @param order     integration order (default 2)
    explicit IntegrationGaussLobattoRegular(unsigned order = 2)
    ...
    {
        this->_n_sampl_pt = static_cast<unsigned>(std::pow(order + 2, N_DIM));
        this->_order = order;
    }
...
#include "IntegrationGaussLobattoRegular-impl.h"
