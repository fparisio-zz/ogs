...
#include "GaussLobattoTet.h"

namespace MathLib
{
template <>
const std::array<std::array<double, 3>, GaussLobattoTet<1>::NPoints>
    GaussLobattoTet<1>::X = {{{{1. / 4., 1. / 4., 1. / 4.}}}};
template <>
double const GaussLobattoTet<1>::W[1] = {1. / 6.};

const std::array<std::array<double, 3>, GaussLobattoTet<2>::NPoints>
    GaussLobattoTet<2>::X = {{{{1. / 4., 1. / 4., 1. / 4.}},
                              {{1. / 6., 1. / 6., 1. / 6.}},
                              {{1. / 2., 1. / 6., 1. / 6.}},
                              {{1. / 6., 1. / 2., 1. / 6.}},
                              {{1. / 6., 1. / 6., 1. / 2.}}}};
double const GaussLobattoTet<2>::W[5] = {-2. / 15., 0.075, 0.075, 0.075, 0.075};

static std::array<std::array<double, 3>, GaussLobattoTet<3>::NPoints>
initGLTet3X()
{
...
const std::array<std::array<double, 3>, GaussLobattoTet<3>::NPoints>
    GaussLobattoTet<3>::X = initGLTet3X();
...
double const GaussLobattoTet<3>::W[GaussLobattoTet<3>::NPoints] = {
